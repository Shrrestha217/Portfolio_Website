...
const Projects = ({ onScrollToCertificate }) => {
    return (
        <section id="projects" className="bg-white dark:bg-[#111] text-[#0d3b66] dark:text-white pb-20 pt-28 px-4 transition-colors duration-300">
            ...
            <div className="relative flex flex-col items-center">
                ...
                <div className="flex flex-col gap-20 relative z-10 w-full">
                    {projects.map((project, index) => {
                        const isLeft = index % 2 === 0;
                        const card = (
                            <div className="bg-blue-100/60 dark:bg-[#1a1a1a] border border-blue-300/60 dark:border-white/10 rounded-xl p-6 shadow-lg hover:shadow-[0_0_20px_#1dbce3] w-full max-w-md text-base hover:scale-105 transition-transform">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {project.tags.map((tag, i) => (
                                        <span
                                            key={i}
                                            className="text-[10px] px-2 py-1 bg-blue-200/30 dark:bg-white/10 border border-blue-300/40 dark:border-white/10 rounded-full text-blue-900 dark:text-white/80"
                                        >
                                            {tag}
                                        </span>
                                    ))}
                        ...
                        const image = (
                            <div className="relative w-[420px] group cursor-pointer">
                        ...
                        return (
                            ...
                            >
                                ...
                                {isLeft ? (
                                    <>
                                        <div className="md:w-1/2 px-6 flex justify-center -translate-x-6">{card}</div>
                                        <div className="md:w-1/2 px-6 flex justify-center translate-x-6">{image}</div>
                                    </>
                                ) : (
                                    <>
                                        <div className="md:w-1/2 px-6 flex justify-center -translate-x-6">{image}</div>
            ...
            <div className="mt-10 flex flex-col items-center group">
                <button onClick={onScrollToCertificate}
